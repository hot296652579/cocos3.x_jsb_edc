...
def decrypt(is_zip, input_key, input_jsc_path):
    ...
    if jsc_path == '':
        jsc_path = input('please input your .jsc path:')
    ...
    key = input_key
    if key == '':
        key = input('please input your encrypt key:')

    if key == '':
        print("error: your key is empty.")
        return False

    # Convert key to bytes for Python 3 compatibility
...
def encrypt(is_zip, input_key, input_js_path):
    ...
    if os.path.exists("encryptOutput") is True:
        shutil.rmtree("encryptOutput")
    os.mkdir("encryptOutput")

    js_path = input_js_path
    if js_path == '':
        js_path = input('please input your .js path:')
    ...
    if is_zip is True:
        ...
        import shutil
        shutil.copy(js_path, prefix + '/encrypt.js')
    ...
    shutil.copy(final_jsc_name, "encryptOutput/projectChanged.jsc")
